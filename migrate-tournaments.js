            ...
            try {
                ...
                for (const gameFile of gameFiles) {
                    ...
                    try {
                        ...
                        // update round dictionary if needed
                        let packetRow = findPacketStatement.get(questionSetEditionId, packetName);
                        if (!packetRow) {
                            // try again without any dash between "Satellite" and "Packet"
                            packetRow = findPacketStatement.get(
                                questionSetEditionId,
                                packetName.replace(/\s*[-–]\s*/, ' ')
                            );
                        }
                        if (!packetRow) throw new Error(`Packet ${packetName} not found`);
                        const { id: packetId } = packetRow;
